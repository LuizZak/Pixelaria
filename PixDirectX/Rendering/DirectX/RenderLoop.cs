...
using static PixDirectX.Rendering.DirectX.Win32Native;
    ...
    public class RenderLoop : IDisposable
    {
        ...
        public unsafe bool NextFrame()
        {
            ...
            if (_isControlAlive)
            {
                ...
                {
                    IntPtr localHandle = _controlHandle;
                    if (localHandle != IntPtr.Zero)
                    {
                        // Previous code not compatible with Application.AddMessageFilter but faster then DoEvents
                        NativeMessage msg;
                        while (PeekMessageW(&msg, IntPtr.Zero, 0, 0, 0))
                        {
                            if (GetMessageW(&msg, IntPtr.Zero, 0, 0) == -1)
                            {
                            ...
                            var message = new Message() { HWnd = msg.hwnd, LParam = msg.lParam, Msg = (int)msg.msg, WParam = new IntPtr((long)msg.wParam) };
        ...
        public static unsafe bool IsIdle
        {
            ...
            {
                NativeMessage msg;
                return PeekMessageW(&msg, IntPtr.Zero, 0, 0, 0) == new RawBool(0);
            }
        }
    }
}
